...
//        self.view.backgroundColor = hexStringToUIColor(hex: statusBarColor)
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        BMHAfrica.webView.setNeedsLayout()
    }
    
    func initWebView() {
        BMHAfrica.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(BMHAfrica.webView);
        
        BMHAfrica.webView.uiDelegate = self;

        
        BMHAfrica.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    @objc func loadRootUrl() {
        BMHAfrica.webView.load(URLRequest(url: rootUrl));
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            BMHAfrica.webView.isHidden = false;
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                BMHAfrica.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(BMHAfrica.webView.estimatedProgress);
    ...
    deinit {
        BMHAfrica.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: BMHAfrica.webView)
        }
